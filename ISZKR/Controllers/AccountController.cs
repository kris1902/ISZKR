...
namespace ISZKR.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(OutsideViewModel model, string returnUrl)
        {
            ...
            var result = await SignInManager.PasswordSignInAsync(model.LoginAndRegisterViewModel.loginViewModel.Email, model.LoginAndRegisterViewModel.loginViewModel.Password, model.LoginAndRegisterViewModel.loginViewModel.RememberMe, shouldLockout: false);
            switch (result)
            {
                ...
                case SignInStatus.RequiresVerification:
                    return RedirectToAction("SendCode", new { ReturnUrl = returnUrl, RememberMe = model.LoginAndRegisterViewModel.loginViewModel.RememberMe });
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(OutsideViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.LoginAndRegisterViewModel.registerViewModel.Email, Email = model.LoginAndRegisterViewModel.registerViewModel.Email };
                var result = await UserManager.CreateAsync(user, model.LoginAndRegisterViewModel.registerViewModel.Password);
                if (result.Succeeded)
                {
